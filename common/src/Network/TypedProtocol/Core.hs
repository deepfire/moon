...
data Peer ps (pr :: PeerRole) (st :: ps) m a where
  ...
  -- | Suspend the state machine with a result. A state token must be provided
  -- from the 'WeHaveAgency' states, to show that this is a state in which we can
  -- suspend.
  --
  -- Example:
  --
  -- > Suspend (ClientAgency TokIdle)
  -- >         MsgSuspend
  -- >         (Suspend TokSuspend result)
  --
  Suspend :: !(TheyHaveAgency pr st)
          -> a
          -> Peer ps pr st m a
